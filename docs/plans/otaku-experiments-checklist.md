...
> **게시판 UX:** 작품·채널별 게시판에서는 주제가 게시판에 고정되므로 **태그 UI 없음**. 글 분류·스포는 **말머리**로 처리. `tags`/`post_tags` DB는 검색 별칭·캐릭터·CP 등 **게시판 밖/교차 검색** 확장용으로 유지.

- [experiment/a3-tag-system] 상태: 브랜치·원격 DB 마이그레이션 적용됨 (2026-05-13).
- [x] `tags`, `tag_aliases`, `post_tags` 테이블 + RLS (`experiment_a3_tag_system`)
- [x] `boards.tag_policy` jsonb
- [ ] 게시판 외 화면에서 태그 입력·검색·별칭 연동 (미착수)
- [ ] 초성(한글) 자동완성
- [ ] 채택 기준 측정: (게시판 제외 범위로 재정의 필요 시) 태그 부착률·클릭 전환
- [ ] 결정: 채택 / 폐기 (날짜:        / 사유:        )
...
- [ ] 상태: `[ ]`
- [x] 글쓰기 말머리에 `스포`·`스포일러` 선택지 (`app/board/.../write`)
- [x] 상세: 말머리 스포일러 시 안내 문구 (`postHasSpoilerTitlePrefix`)
- [x] 글쓰기 "스포일러 포함" 단독 체크박스 (선택)
- [x] 저장 시 해당 태그/말머리 자동 부착 (A3 통합 태그 또는 기존 말머리 중 택1)
- [ ] 목록 카드: `스포` 칩 + 썸네일 흐리게 + 본문 미리보기 숨김 (현재 미적용: 일반 글과 동일 노출)
- [x] 상세 진입 시 짧은 "스포일러 주의" 안내(닫기 가능)
