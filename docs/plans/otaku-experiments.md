...
| A4 | 스포일러 정책/인라인 마스킹 | 오타쿠 정체성 | P1 | `experiment/a4-spoiler-policy` | A3 |
...
- **가설:** 신작·최신화 토론에서 스포일러 정책이 명문화되어야 사람이 모인다. "회차 단위 마스킹"과 "본문 내 인라인 블러"가 둘 다 필요하다.
- **데이터:** `posts.spoiler_scope text`(`none|episode|volume|finale|crossmedia`), `posts.spoiler_episode int|null`. 본문 블록에 `{ type: "spoiler", reveal: "click|hover", text|inline }` 추가.
- **화면:** 작성 시 스포일러 범위 선택 강제(특정 게시판 한정), 목록에서 스포일러 글 썸네일 자동 블러, 본문 내 인라인 블러 클릭 시 노출, "n화까지 본 사람만" 토글로 미달 사용자 진입 시 경고.
- **에디터:** Tiptap 인라인 마크 추가.
- **브랜치:** `experiment/a4-spoiler-policy`
- **채택 기준:** 스포일러 정책 게시판에서 스포일러 미표기 신고 사유 발생률이 도입 후 30일 내 1/3 이하로 감소.
- **폐기 시:** 인라인 블러 마크는 유지(에디터 자산), 게시판 정책 강제만 끈다.
...
| `character-community.md` 7장 | 스포일러 가드 정책 (A4 채택 후) |
