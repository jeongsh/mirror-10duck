...
- 오늘 한 일:
  - 캐릭터 라이브러리/업로드 관련 컴포넌트 구조를 `components/character` 중심으로 정리하고 분리된 패널 구성을 진행했다.
  - 라이브러리 상세 진입 페이지(`app/library/[id]/page.tsx`)와 프로필 페이지(`app/profile/page.tsx`) 작업 흐름을 추가해 캐릭터 관리 동선을 확장했다.
  - 캐릭터 상태 관리를 위한 스토어 분리 작업(`store/useCharacterStore.ts`)과 관련 문서 업데이트를 병행했다.
- 남은 일:
  - 중복 경로/파일 정리(슬래시 표기 차이 포함)와 import 정합성 점검이 필요하다.
  - 캐릭터 업로드/미리보기/라이브러리 패널 간 상태 동기화 검증 및 예외 케이스 보완이 필요하다.
- 내일 할 일:
  - 캐릭터 관련 신규 컴포넌트들을 실제 라우트에 연결하고 사용자 흐름 기준으로 E2E 점검한다.
  - 문서와 코드 기준 경로를 통일하고, 빌드/린트 통과 기준으로 리팩터링 마무리한다.