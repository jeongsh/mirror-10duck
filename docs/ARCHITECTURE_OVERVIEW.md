...
- 현재는 아래 항목 중심으로 수정 가능:
  ...
  - 기본 뷰(scale/x/y)
  - 전역 기본 반응 대사(핵심 감정/액션만)
  - 모델 원본 후보 기반 상황별 표정+모션 매핑
- 활성 캐릭터일 때 캔버스의 현재 위치/스케일을 `defaultView`로 저장할 수 있습니다.
- 페이지 입장 대사는 이 화면에 계속 누적하지 않고, 추후 페이지별 프리셋 편집으로 분리합니다.

### `components/character/MappingPanel.tsx`
- 로그인, 로그아웃, 알림 공통, 글쓰기 중, 기본 복귀 같은 상황 단위로 표정과 모션을 한 번에 매핑하는 패널입니다.
- 댓글/답글/리액션/팔로우/시스템 알림은 말풍선 텍스트와 추후 사운드는 각각 나눌 수 있지만, Live2D 모션은 알림 공통 매핑 하나를 공유합니다.
- `model3.json`의 Expressions/Motions/HitAreas를 읽어 캐릭터가 실제 가진 후보만 선택지로 보여줍니다.
- 미리보기 모델은 매핑 행 옆에 붙어 있는 관리 화면 상단에 배치되어, `[미리보기]`를 누르면 스크롤 이동 없이 표정+모션 조합을 바로 확인할 수 있습니다.
- 미리보기 모델을 클릭하면 감지된 HitArea가 최근 클릭 로그와 매핑 행 하이라이트로 표시됩니다.
- 개별 감정->표정, 액션->모션 매핑은 호환/세밀 조정용 고급 섹션으로 접어 둡니다.
...
- 현재는 레거시 `:sticker/{characterId}/{emotion}:` 토큰을 넣는 임시 피커입니다.
- 제품 기준 스티커는 Live2D 표정 지원 여부와 분리된 별도 에셋입니다.
- 정식 피커는 사용자가 직접 등록하거나 AI 생성한 `stickers` / `sticker_assets` 목록을 이미지 그리드로 보여주는 구조로 교체합니다.
...
6. 글쓰기/댓글 입력 시 현재 `StickerPicker` 는 레거시 토큰을 넣지만, 정식 구조에서는 `sticker_assets` 목록을 스티커 소스로 사용
