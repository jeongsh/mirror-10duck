...
type ProfileContentTab = "all" | "board" | "feed" | "comments";

function profileTabFromSearch(searchParams: { get: (key: string) => string | null }): ProfileContentTab {
  const t = searchParams.get("tab");
  if (t === "comments") return "comments";
  if (t === "board") return "board";
  if (t === "feed") return "feed";
  // 예전 `tab=posts` 링크는 전체(게시글) 탭으로 취급
  return "all";
}

function postDetailHref(post: CommunityPost): string | null {
  if (post.source_type === "BOARD" && post.boards?.slug) {
    return `/board/${post.boards.slug}/${post.id}`;
  }
...
export default function UserFeedPage() {
  ...
  const navigateTab = useCallback(
    (t: ProfileContentTab) => {
      const base = `/user/${encodeURIComponent(handle)}`;
      if (t === "all") router.replace(base, { scroll: false });
      else router.replace(`${base}?tab=${t}`, { scroll: false });
    },
    [handle, router],
  );

  const filteredPosts = useMemo(() => {
    if (activeTab === "board") return posts.filter((p) => p.source_type === "BOARD");
    if (activeTab === "feed") return posts.filter((p) => p.source_type === "FEED");
    return posts;
  }, [posts, activeTab]);
  ...
  const fetchUserData = useCallback(async () => {
    ...
    let postsQuery = supabase
      .from("posts")
      .select("*, profiles(*), boards(slug, name)")
    ...
    const { data: commentsData } = await supabase
      .from("comments")
      .select("id, post_id, content, created_at, posts(title, source_type, boards(slug))")
      .eq("author_id", profile.user_id)
      .order("created_at", { ascending: false })
      .limit(50);
    setComments((commentsData as UserCommentActivity[] | null) ?? []);
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-col border-x border-dashed border-gray-500 bg-white/40 min-h-screen">
      ...
      {/* 탭: 전체 / 게시판 / 피드 / 댓글 */}
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-b border-dashed border-gray-500 bg-white/90 px-4 py-3">
        <button
          type="button"
          onClick={() => navigateTab("all")}
          className={`pb-2 text-sm font-bold ${
            activeTab === "all" ? "border-b-2 border-gray-800 text-gray-900" : "text-gray-500"
          }`}
        >
          게시글
        </button>
        <button
          type="button"
          onClick={() => navigateTab("board")}
          className={`pb-2 text-sm font-bold ${
            activeTab === "board" ? "border-b-2 border-gray-800 text-gray-900" : "text-gray-500"
          }`}
        >
          게시판
        </button>
        <button
          ...
          className={`pb-2 text-sm font-bold ${
            activeTab === "feed" ? "border-b-2 border-gray-800 text-gray-900" : "text-gray-500"
          }`}
        >
          피드
        </button>
        <button
          type="button"
          onClick={() => navigateTab("comments")}
          className={`pb-2 text-sm font-bold ${
            activeTab === "comments" ? "border-b-2 border-gray-800 text-gray-900" : "text-gray-500"
          }`}
        >
          댓글 활동
        </button>
      ...
      <section className="flex flex-col">
        {activeTab !== "comments" ? (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {(() => {
                      ...
                      const bodyBlock = isBoardShareFeed ? (
                      ...
                      <>
                        {post.source_type === "BOARD" ? (
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-gray-500">
                            <span className="border border-dashed border-gray-300 px-1.5 py-0.5">피드</span>
                          </div>
                        )}
                      ...
                      return (
                        <Link href={href} prefetch={false} className="block text-inherit no-underline">
                          {bodyBlock}
                        </Link>
                      );
                    })()}
          ...
          <div className="p-8 text-center text-sm text-gray-500">아직 작성한 댓글이 없습니다.</div>
        ) : (
          comments.map((comment) => (
            <article
              ...
              className="border-b border-dashed border-gray-500 bg-white/70 px-4 py-3 transition-colors hover:bg-gray-50"
            >
              <p className="mb-1 text-xs text-gray-500">{formatCommunityDate(comment.created_at)}</p>
              <p className="line-clamp-3 text-sm text-gray-800">{comment.content || "(스티커 댓글)"}</p>
              <button
                type="button"
                onClick={() => {
                  const slug = comment.posts?.boards?.slug;
                  const st = comment.posts?.source_type;
                  if (slug) {
                    router.push(`/board/${slug}/${comment.post_id}#comment-${comment.id}`);
                  } else if (st === "FEED") {
                    router.push(`/feed?post=${comment.post_id}#comment-${comment.id}`);
                  }
                }}
                disabled={!comment.posts?.boards?.slug && comment.posts?.source_type !== "FEED"}
                className="mt-2 text-xs text-gray-500 hover:text-gray-800 hover:underline disabled:pointer-events-none disabled:opacity-40"
              >
                원문으로 이동
              </button>
            </article>
